...
export class Game {
    ...
    async addPlayer(userId: string, username: string, interaction: any, isSilent = false) {
        if (this.players.some(p => p.id === userId)) {
            if (!isSilent) return interaction.reply({ content: '❌ You are already in the game!', ephemeral: true });
            return;
        }

        if (this.state !== GameState.LOBBY) {
            if (!isSilent) return interaction.reply({ content: '❌ Game has already started!', ephemeral: true });
            return;
        }

        if (this.players.length >= 10) {
            if (!isSilent) return interaction.reply({ content: '❌ Lobby is full!', ephemeral: true });
            return;
        }
    ...
    getCardImageUrl(card: Card | null): string {
        ...
        if (card.value === 'skip') text = '⊘';
        if (card.value === 'reverse') text = '⇄';
    ...
    async handleSayUno(interaction: any) {
        ...
        if (!player) return interaction.reply({ content: '❌ Not in game.', ephemeral: true });
    ...
    async handleLeave(interaction: any) {
        ...
        if (playerIndex === -1) return interaction.reply({ content: '❌ You are not in the game.', ephemeral: true });
    ...
    async executePlay(interaction: any, player: Player, card: Card, cardIndex: number, wildColor?: string) {
        ...
        if (player.hand.length === 0) {
            ...
            const scoreboard = rankedPlayers.map((p, i) => {
                let medal = '🎲';
                let points = '';
                if (i === 0) { medal = '🥇'; points = '(+10 pts)'; }
                else if (i === 1) { medal = '🥈'; points = '(+5 pts)'; }
                else if (i === 2) { medal = '🥉'; points = '(+3 pts)'; }
